...
- **DAW-style loop activation** — `setLoopEnabled()` and `setLoopRegion()` during playback only activate Transport loop if the current position is inside the loop region. If past `loopEnd`, the intent is stored (`_isLoopEnabled = true`) but Transport loop stays off — playback continues to the end. `play()` has its own position check (can't use the helper since `_isPlaying` is still `false` at that point).
