...
**Deferred loop enable (confirmed root cause):** Setting `transport.loop = true` before `transport.start()` causes an immediate wrap to loopStart on the first tick cycle. Root cause: `Clock._loop()` processes ticks from `_lastUpdate` to `now()`. After stop/start cycles, `_lastUpdate` is stale (from the previous cycle). `transport.start(time, offset)` calls `setTicksAtTime(ticks, time)`, but ticks generated for `tickTime < time` use the **previous** TickSource state ("ghost ticks"). If those stale ticks `>= _loopEnd` AND `_loop.get(tickTime)` returns `true`, `_processTick` wraps to loopStart before the intended offset takes effect. Reproducible with rapid stop/start cycles + small loop regions (see `debug-loop.html` test 6). **Fix:** Always start Transport with `loop = false`. After `transport.start()`, defer `transport.loop = true` via `setTimeout(0)`. The deferred timeout ensures `_loop.get(tickTime)` returns `false` for the stale-tick range, preventing the wrap. Cleaned up in `pause()`, `stop()`, and `dispose()`. See [Tone.js #1419](https://github.com/Tonejs/Tone.js/issues/1419).
