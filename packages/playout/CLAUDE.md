...
**Deferred loop enable:** Setting `transport.loop = true` before `transport.start()` causes an immediate wrap to position 0 on the first tick cycle. Static analysis of `_processTick` suggests it shouldn't happen (`_loop.get(tickTime)` and `ticks >= _loopEnd` checks should both behave correctly), but it does — likely due to subtle timing interactions between `this.now()` lookahead, TickSource state accumulation across stop/start cycles, and Clock's `_lastUpdate` range. **Fix:** Always start Transport with `loop = false`. After `transport.start()`, defer `transport.loop = true` via `setTimeout(0)`. This runs after the current call stack AND any pending audio callbacks, so the initial `_processTick` batch sees `loop = false`. The deferred timeout is cleaned up in `pause()`, `stop()`, and `dispose()`.
